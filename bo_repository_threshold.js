...
async function getWFRepos() {
  try {
    ...
    console.log(`Total de repositorios en ${ORG_NAME}: ${repos.length}`);
    console.log(`Repositorios de BackOffice (WF_ o ORA_): ${numRepos}`);

    // Imprimir los nombres de los repositorios de BackOffice
    console.log('Repositorios de BackOffice:');
    reposBackoffices.forEach(repo => {
      console.log(`- ${repo.name}`);
    });

    return reposBackoffices;
  } catch (error) {
